...
const jsonl_filename = "../crawler/briankeng-2023-06-09.jsonl";
...
assert(rawDocs.length == urlDocs.length, "rawDocs and urlDocs must be the same length");
assert(rawDocs.length == titleDocs.length, "rawDocs and titleDocs must be the same length");

// add url metadata to rawDocs
for (let i = 0; i < rawDocs.length; i++) {
    rawDocs[i].metadata["url"] = urlDocs[i].pageContent;
    rawDocs[i].metadata["title"] = titleDocs[i].pageContent;
}
